...
ComponentCache::ComponentCache(DataAndStatistics &statistics,
                               SolverConfiguration &config) : statistics_(statistics), config_(config)
{
}
...
void ComponentCache::init(Component &super_comp)
{
  ...
  CacheableComponent &packed_super_comp = *new DifferencePackedComponent(super_comp);
  my_time_ = 1;

  entry_base_.clear();
  entry_base_.reserve(2000000);
  entry_base_.push_back(new DifferencePackedComponent()); // dummy Element
  table_.clear();
  ...
  {
    cout << endl
         << " WARNING: Maximum cache size larger than free RAM available" << endl;
...
bool ComponentCache::deleteEntries()
{
  ...
  statistics_.num_cached_components_ = entry_base_.size(); //TODO: Shouldn't this be size - 2?? -VincentDerk.
  compute_byte_size_infrasture();
