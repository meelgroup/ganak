...
public:
  ...
  unsigned raw_data_byte_size() const {
    return data_size()* sizeof(unsigned) + model_count_.get_mpf_t()->_mp_size * sizeof(mp_limb_t);
  }

  unsigned raw_data_byte_size_CLHASH() const {
    return hack_* sizeof(uint64_t) + model_count_.get_mpf_t()->_mp_size * sizeof(mp_limb_t);
  }
