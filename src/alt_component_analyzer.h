...
public:
    ...
    inline ClHashComponent *makeSTDClHashFromArcheType(vector<void *> &random, Component *comp) {
        return archetype_.constructSTDClHashComp(random, comp, literal_pool_, map_clause_id_to_ofs_);
    }

    inline SparseGraphCacheComponent *makeIsoCompFromArcheType(Component *comp) {
        return archetype_.constructIsoComp(comp, literal_pool_, map_clause_id_to_ofs_, literals_);
    }
...
private:
    ...
    vector<ClauseOfs> map_clause_id_to_ofs_;
